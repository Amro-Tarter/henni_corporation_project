...
import { collection, query, getDocs, onSnapshot, doc, setDoc, serverTimestamp, where, deleteDoc } from "firebase/firestore"; // Import 'where' and 'deleteDoc'
    ...
    const handleDeleteDonation = async (donationId) => {
        ...
        if (window.confirm("האם אתה בטוח שברצונך למחוק תרומה זו?")) {
            try {
                ...
                toast.error("אירעה שגיאה במחיקת התרומה. אנא נסה שנית.");
            }
        }
    };
                    ...
                    {donationsList.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {donationsList.map(donation => (
                                <div key={donation.id} className="bg-white p-6 rounded-lg shadow-md border border-gray-200 relative"> {/* Added relative for positioning */}
                                    <button
